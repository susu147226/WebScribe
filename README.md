...
- **按文档分组合并展示**：同属一份文档的链接收在一个可折叠的分组里，
  ...
  不必靠滚动去找
- 提供「只看问题」筛选
- 链接数量上限可选 **10 / 50 / 100 / 150 / 200** 档，并按同一站点最多的条数估算最短耗时
...
- **标题按内容结构判断，不依赖 HTML 标签**。文档标题未必是 `<h1>`——
  有的站点用 `<div class="title">`、`<span class="title">`，荣耀的文档站则把
  正文标题与小节标题都写成 `<h3>`。程序会排除导航、菜单、弹窗等外壳区域，
  按「页面自己的标题」取值；嵌套候选只取最内层，避免把旁边的按钮文字
  （如「智能摘要」）一并带上
- **正文里的各级标题会被保留**。不少文档站把正文标题的 id 命名为 `header-1`、
  `header-2`……，而 Readability 会把 id 含 `header` 的元素当成页头导航删掉 ——
  实测某文档页 12 个小节标题因此全部消失。程序会在交给 Readability 之前
  把这类标题的 id 换成不含触发词的新值，并同步页内锚点
- Markdown 保留 H1–H6、段落、粗体、斜体、链接、图片、有序/无序列表、表格、引用、行内代码、代码块
- **短正文也能正常提取**：论文摘要、小说短章、诗歌等不足一两百字的正文不会被误判为「无法获取正文」
...
- **内容无变化则跳过**：再次抓取同一链接时，若抓到的正文与上次完全一致，
  ...
  汇总提示共跳过多少条。
  **跳过有个前提**：上次产出的文档必须仍在当前保存目录里 ——
  换了保存目录、或文件被删除/移动时会重新抓取，不会让您拿到空结果
...
遇到验证码、Cloudflare Challenge、**WAF JS 挑战**（如起点的小说章节）、
**中文人机验证**（如知网的「请完成安全验证」）、403 访问拒绝等情况时，程序会：
